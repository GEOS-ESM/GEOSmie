...
class OPTICS(object):
    def __init__(self,inFile,irh=None,ibin=None,iwav=None):
        ...
        if ibin is None:
            self.ibin = np.arange(self.optics.sizes['radius'])
        else:
            self.ibin = ibin
        ...
        self.coords = {'rh': self.optics['rh'][self.irh],
                       ...
                       'bin': self.optics['radius'][self.ibin],
                      } 
        ...
        self.bext = self.bext.rename({'radius': 'bin'})
        self.bsca = self.bsca.rename({'radius': 'bin'})
        self.bbck = self.bbck.rename({'radius': 'bin'})
    ...
    def plot(self, wavelength=550, ibin=None):
        ...
        if ibin is None:
            ibin = np.arange(self.bext.sizes['bin'])

        for bin in ibin:
            fig, axes = plt.subplots(nrows=3, ncols=2, figsize=(12, 16))

            bext = self.bext.isel(wavelength=iwav,bin=bin)
            bsca = self.bsca.isel(wavelength=iwav,bin=bin)
            bbck = self.bbck.isel(wavelength=iwav,bin=bin)
            b450 = self.bext.isel(wavelength=i450,bin=bin)
            b900 = self.bext.isel(wavelength=i900,bin=bin)
            ...
            bext = self.bext.isel(rh=0,bin=bin)
            bsca = self.bsca.isel(rh=0,bin=bin)
            ssa00 = bsca/bext
            ax.plot(self.wavelengths, bext/1000., label='RH=0%')
            bext = self.bext.isel(rh=i40,bin=bin)
            bsca = self.bsca.isel(rh=i40,bin=bin)
            ssa40 = bsca/bext
            ax.plot(self.wavelengths, bext/1000., label='RH=40%')
            bext = self.bext.isel(rh=i80,bin=bin)
            bsca = self.bsca.isel(rh=i80,bin=bin)
            ssa80 = bsca/bext
            ax.plot(self.wavelengths, bext/1000., label='RH=80%')
            bext = self.bext.isel(rh=i95,bin=bin)
            bsca = self.bsca.isel(rh=i95,bin=bin)
            ssa95 = bsca/bext
