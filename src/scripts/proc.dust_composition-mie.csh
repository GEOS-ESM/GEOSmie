...
# setup environment
setenv SRC_DIR /rvol/vole/colarco/sandbox/GEOSmie
...
# Link the desired files
  ...
  sed 's/SITE/'"$site"'/g'  ${PWD}/geosparticles/experimental/dust_composition/du-lognorm-mie.json > $ver/DU_mie_$site.$ver.json
  end
...
# All wavelengths
  foreach site (`echo $sites`)
   ./runoptics.py --name $ver/DU_mie_$site.$ver.json --dest=$ver> $ver/optics_DU_mie_$site.$ver.txt &
  end
  wait
...
# Make plots
  mkdir -p plots
  foreach site (`echo $sites`)
   ./plotoptics_bins.py --name ./AerosolOptics/$ver/x/optics_DU_mie_$site.$ver.nomom.nc4
  end
