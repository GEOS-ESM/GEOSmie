...
# Link the desired files
  ...
  sed 's/SITE/'"$site"'/g'  ${PWD}/geosparticles/experimental/dust_composition/du-lognorm.json > $ver/DU_grasp_$site.$ver.json
  end
...
# All wavelengths
  foreach site (`echo $sites`)
   ./runoptics.py --name $ver/DU_grasp_$site.$ver.json --dest=$ver> $ver/optics_DU_grasp_$site.$ver.txt &
  end
  wait
